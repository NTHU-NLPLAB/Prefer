...
def cache_similary_word(word, LIMIT=15):
    db_path = os.path.join(DB_DIR, "cache_similar_word.db")
...
def cache_single_translate(word, LIMIT=15):
    db_path = os.path.join(DB_DIR, "cache_single_translate.db")
    con = sqlite3.connect(db_path)
    cur = con.cursor()
...
def paraphrase_by_translation(phrase, limit=10):
    ...
    for ch in ch_trans:
        ...
        for en in en_trans:
            ...
            if en not in en_translation_dic:
...
def find_candidate_phrase(content_words):
    db_path = os.path.join(DB_DIR, "ACL_only_phrases.db")
    con = sqlite3.connect(db_path)
    cur = conn.cursor()
    all_phrase = []
