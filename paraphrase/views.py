...
def get_simple_paraphrase( request ):
	...
	return HttpResponse( simplejson.dumps( dict([ ( v[1], v[0] ) for v in func_paraphrase.paraphrase_by_bleu( qphrase )[0] ] ))) 
...
def get_paraphrase2( request ):
	if not request.GET.has_key("qphrase"):
